...
.SDK+lib+API+python+max+entrypoints+pipelines.venv/bin/python -m pip install --upgrade --ignore-installed pip python-lsp-server ruff python-lsp-ruff mypy pylsp-mypy pytest pytest-asyncio debugpy py-cpuinfo hypothesis accelerate nvitop datasets types-simplejson types-PyYAML types-protobuf types-requests types-tqdm types-tensorflow torchvision
rm get-pip.py
